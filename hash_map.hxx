...
public:
	...
	T2 &	operator[](T1 key) { return (find(key)); };
	void	del_key(T1 key)
	{
		size_t							h;
		T3								hash;
	...
	int		getCountElem(void) { return (count_of_elem); };
	float	level_current(void) { return (count_of_elem / count_of_list); };
	float	getK(void) { return (k); };
	void	change_k(float new_k) { k = new_k; };
	void	print(void)
	{
		for (int i = 0; i < hash_table.size(); i++)
		{
